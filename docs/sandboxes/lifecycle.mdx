...
`msb modify` and the SDK `modify()` methods change a sandbox in place: CPUs, memory, env vars, labels, workdir, and secrets. Every change is planned first, and apply is all-or-nothing.

CPU and memory apply live while they stay within the `max_cpus` / `max_memory` ceilings chosen at create time. The ceilings default to the initial values, so set them higher at create time if the sandbox may need to grow.
...
Preview any change with a dry run, and pick a policy for changes that cannot apply live: `--next-start` saves them for the next boot, `--restart` applies them now (`policy` in the SDKs). Modifying a stopped sandbox never starts it.

See [`msb modify`](/cli/sandbox-commands#msb-modify) for all flags and caveats.
