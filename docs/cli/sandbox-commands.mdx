...
Env and workdir changes apply to future execs only; running processes keep their current environment.

`--secret NAME@HOST` adds or rotates a secret from the host environment variable of that name; the value never appears in the command. Inline `NAME=VALUE@HOST` is rejected, same as on `msb create`, since it would leak into shell history. To pass a raw value, use an SDK.
