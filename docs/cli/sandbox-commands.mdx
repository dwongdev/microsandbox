...
A live resize may not apply instantly; `--format json` reports a per-resource `resize_status`. See [Config](/sandboxes/configuration) for the change model, or use `msb modify --help` for the full flag list.
